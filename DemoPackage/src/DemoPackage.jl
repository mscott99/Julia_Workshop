...
const ϵ = DualNumber(false, true)  # now we can do 1.0 + 2ϵ!
...
/(a::DualNumber, b::DualNumber) = DualNumber(real(a) / real(b), ((dual(a) * real(b)) - (real(a) * dual(b))) / (real(b)^2))
...
"""
derivative(p::Polynomial, at::Real) = (at + one(at)ϵ) |> p |> dual
